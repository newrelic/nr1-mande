export default {
  ...
  metrics: [
    ...
    },
    {
      ...
      query: {
        nrql: `FROM PageAction, MobileVideo, RokuVideo SELECT filter(count(*), WHERE actionName = 'CONTENT_BUFFER_START' and contentPlayhead > 0) / filter(count(*), WHERE actionName IN ('CONTENT_START', 'CONTENT_NEXT')) as 'result'`,
        lookup: 'result',
      },
      detailConfig: [
        {
    ...
    //   detailConfig: [
    //     {
    //       nrql: `FROM PageAction, MobileVideo, RokuVideo SELECT filter(uniqueCount(viewId), WHERE actionName = 'CONTENT_RENDITION_CHANGE') / filter(uniqueCount(viewId), WHERE actionName IN ('CONTENT_START','CONTENT_NEXT')) * 100 as '%' `,
    //       columnStart: 1,
    //       columnEnd: 3,
    //       chartSize: 'small',
    //       chartType: 'billboard',
    //       title: 'Views with Renditions',
    //       useSince: true,
    //     },
    //     {
